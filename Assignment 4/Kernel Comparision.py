...
for reps in [x*3 for x in range(1,10,2)]:
    for regwgt in [(10**x) for x in range(-3,1)]:
        weights = None

        K = polynomialKernel(trainset[0], c)
        numsamples = xtrain.shape[0]
        weights = np.dot(np.dot(np.linalg.inv(np.add(np.dot(K.T,K)/numsamples,regwgt*np.identity(K.shape[1]))), K.T),yt)/numsamples
        Ktest = polynomialKernel(testset[0], c)
        ytest = np.dot(Ktest, weights)
        ytest[ytest > 0] = 1
        ytest[ytest < 0] = 0
        accuracy = getaccuracy(testset[1], ytest)
        print "Accuracy for Poly kernel size: " + str(reps) + ", regularizer weight: " + str(regwgt) + " = " + str(accuracy)

        K = linearKernel(trainset[0], c)
        numsamples = xtrain.shape[0]
        weights = np.dot(np.dot(np.linalg.inv(np.add(np.dot(K.T,K)/numsamples,regwgt*np.identity(K.shape[1]))), K.T),yt)/numsamples
        Ktest = linearKernel(testset[0], c)
        ytest = np.dot(Ktest, weights)
        ytest[ytest > 0] = 1
        ytest[ytest < 0] = 0
        accuracy = getaccuracy(testset[1], ytest)
        print "Accuracy for Lin kernel size: " + str(reps) + ", regularizer weight: " + str(regwgt) + " = " + str(accuracy)

        K = gaussianKernel(trainset[0], c)
        numsamples = xtrain.shape[0]
        weights = np.dot(np.dot(np.linalg.inv(np.add(np.dot(K.T,K)/numsamples,regwgt*np.identity(K.shape[1]))), K.T),yt)/numsamples
        Ktest = gaussianKernel(testset[0], c)
        ytest = np.dot(Ktest, weights)
        ytest[ytest > 0] = 1
        ytest[ytest < 0] = 0
        accuracy = getaccuracy(testset[1], ytest)
        print "Accuracy for Gaus kernel size: " + str(reps) + ", regularizer weight: " + str(regwgt) + " = " + str(accuracy)
        print ""